...
function GetCallBack({ sourceId }) {
  ...
  return (
    <section
      className="get-a-call-back-section m-auto px-10 py-5  text-white"
      style={{
    ...
    >
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
        ...
        <div className=" md:flex items-center gap-4 md:col-span-8 ">
          <form className="lg:flex items-center gap-4 w-full" onSubmit={submiHandle}>
            ...
            <div className="relative w-full lg:w-1/2">
              ...
              {data.email?.length === 0 && data.mobile?.length === 0 ? (
                <p className="text-sm text-white absolute mb-[-5px]">{showError}</p>
              ) : (
                ''
              )}
              {data.email && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(data.email) && (
                <p className="text-sm text-white absolute mb-[-5px]">Please enter a valid email address.</p>
              )}
            </div>
            ...
            <div className="relative w-full lg:w-1/2">
              ...
              {data.mobile && data.mobile.length !== 10 && (
                <p className="text-sm text-white absolute mb-[-5px]">Please enter a valid 10-digit number.</p>
              )}
            </div>
